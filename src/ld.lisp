  ...
  (with-unique-names (parser context start current end evt read-buffer next-event)
    (let ((func-name (symbol-name name)))
      `(setf (symbol-function (intern ,func-name))
		 ...
		 (declare (type ,(find-type spec :parser-type) ,parser))
		 (declare (type ,(find-type spec :context-type) ,context))
		 (labels ((,next-event ()
...
(defmacro ld-str (name (in) &body spec)
  `(ld ,name (,in)
...
(defmacro ld-stm (name (in) &body spec)
  `(ld ,name (,in)
