...
(defun csv-table-add-field (table s)
  ...
  (let ((row (csv-table-current table)))
    (if (and (csv-table-has-headers table)
	     (zerop (csv-table-row-count table)))
	(vector-push-extend s row)
	(let* ((trans (csv-table-transformers table))
	       (func (if (not (eq *empty-array* trans))
			 (aref trans (fill-pointer row))
			 nil)))
	  (vector-push-extend (if func (funcall func s) s) (csv-table-current table)))))
  table)
